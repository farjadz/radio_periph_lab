...
void print_benchmark(volatile unsigned int *periph_base)
{
	...
	unsigned int elapsed_clocks = 0;
    elapsed_clocks = stop_time - start_time;
	...
	if (elapsed_clocks > 0)
	{
		// Numerator: Number of bytes converted to MegaBytes (/10^6 cancels out)
		float numerator_th = 2048*4;
		// printf("Numerator = %f \n",numerator_th);
		
		// Denominator: # of clocks x clock period (/10^9 cancels out and becomes /10^3)
		float denominator_th = elapsed_clocks*0.008;
		// printf("Denominator = %f \n",denominator_th);
		
		// Throughput
		throughput = numerator_th / denominator_th;
	}	
...
// first, get a pointer to the peripheral base address using /dev/mem and the function mmap
    ...
    printf("\r\n\r\n\r\nLab 6 Farjad Zaheer - Custom Peripheral Demonstration\n\r");
